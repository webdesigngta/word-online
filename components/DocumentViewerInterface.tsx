...
export function DocumentViewerInterface({ toolId }: { toolId: string }) {
  ...
  return (
    <div className="document-viewer-tool">
      ...
      {preview || name ? <div className="document-viewer-actions">
        <div className="document-viewer-file">
          <div className="document-viewer-icon"><FileText /></div>
          <div className="document-viewer-copy"><strong>{name}</strong><span>{status}</span></div>
        </div>
        <div className="document-viewer-buttons">
          <button className="document-viewer-button" type="button" disabled={busy} onClick={() => inputRef.current?.click()}>{busy ? <RefreshCw /> : <FolderOpen />}{busy ? 'Opening…' : 'Open another'}</button>
      ...
      {!preview ? <div className="document-viewer-empty"><button className="document-viewer-button" type="button" disabled={busy} onClick={() => inputRef.current?.click()}>{busy ? <RefreshCw /> : <FolderOpen />}{busy ? 'Opening…' : 'Choose Files'}</button></div> : null}
    </div>
  );
}
