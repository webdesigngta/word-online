...
import { Download, FileUp, RefreshCw, Scissors } from 'lucide-react';
...
export function DocxSplitInterface() {
  ...
  return (
    <div className="fwo-split-tool">
      ...
      <div className="fwo-split-picker"><Scissors /><h2>Split a Word document by headings</h2><p>Create separate DOCX files at Heading 1, Heading 2 or Heading 3 boundaries.</p><button className="fwo-split-button secondary" type="button" onClick={() => inputRef.current?.click()}><FileUp />{file ? 'Choose another DOCX' : 'Choose DOCX file'}</button></div>
      <div className="fwo-split-options"><div><label>Split at<select value={headingLevel} onChange={(event) => setHeadingLevel(Number(event.target.value))}><option value={1}>Heading 1</option><option value={2}>Heading 1–2</option><option value={3}>Heading 1–3</option></select></label><div className="fwo-split-status">{file ? file.name : status}</div></div><button className="fwo-split-button" disabled={!file || busy} type="button" onClick={() => void split()}>{busy ? <RefreshCw /> : <Scissors />}{busy ? 'Splitting…' : 'Split document'}</button></div>
