...
export function MarkdownUtilityInterface({ mode }: { mode: MarkdownUtilityMode }) {
  ...
  return (
    <div className="fwo-md-tool">
      ...
      <div className="fwo-md-top">
        <div className="fwo-md-file">
          <div className="fwo-md-icon"><FileCode2 /></div>
          <div className="fwo-md-copy"><strong>{fileName || (docxInput ? 'DOCX to Markdown' : mode === 'markdown-to-docx' ? 'Markdown to DOCX' : 'Markdown Editor')}</strong><span>{status}</span></div>
        </div>
        <div className="fwo-md-actions">
          ...
          {mode !== 'markdown-editor' ? <button className="fwo-md-btn primary" type="button" disabled={busy} onClick={() => inputRef.current?.click()}>{busy ? <RefreshCw /> : <FolderOpen />}{busy ? 'Working…' : fileName ? 'Open another' : docxInput ? 'Choose DOCX' : 'Choose Markdown'}</button> : null}
        </div>
      </div>

      {mode !== 'markdown-editor' && !fileName ? <div className="fwo-md-empty"><div><FileCode2 /><strong>{docxInput ? 'DOCX to Markdown' : 'Markdown to DOCX'}</strong><div>{docxInput ? 'Convert supported DOCX content into clean Markdown.' : 'Open Markdown, preview it, and export a real DOCX file.'}</div></div></div> : (
