...
import { downloadBlob, readCsv, readXlsx, rowsToCsv, rowsToXlsx } from '@/tools/spreadsheet/tabular';

export type SpreadsheetUtilityMode = 'csv-editor' | 'csv-viewer' | 'csv-to-xlsx' | 'xlsx-to-csv';
...
export function SpreadsheetUtilityInterface({ mode }: { mode: SpreadsheetUtilityMode }) {
  ...
  const editable = mode === 'csv-editor';
  const tableMode = mode === 'csv-editor' || mode === 'csv-viewer';
  ...
  async function open(file?: File) {
    ...
    try {
      const isXlsx = mode === 'xlsx-to-csv';
      if (isXlsx && !/\.xlsx$/i.test(file.name)) throw new Error('Choose an XLSX file.');
      if (!isXlsx && !/\.csv$/i.test(file.name)) throw new Error('Choose a CSV file.');
      if (file.size <= 0 || file.size > 20 * 1024 * 1024) throw new Error('Files must be between 1 byte and 20 MB.');
      const data = isXlsx ? await readXlsx(file) : await readCsv(file);
      const next = normalizedRows(data.rows);
      setRows(next.length ? next : [['']]);
      setSheetName(data.sheetName || 'Sheet1');
      setFileName(file.name);
      setStatus(`${next.length || 1} row${next.length === 1 ? '' : 's'} loaded.`);
    } catch (error) {
      setRows([]);
      setFileName('');
        ...
        <>
          <div className="fwo-sheet-wrap">
            <table className="fwo-sheet-table">
              <thead><tr><th>#</th>{Array.from({ length: columnCount }, (_, index) => <th key={index}>{String.fromCharCode(65 + (index % 26))}{index >= 26 ? Math.floor(index / 26) : ''}</th>)}</tr></thead>
          ...
          <div className="fwo-sheet-summary"><span>{rows.length} rows · {columnCount} columns</span>{editable ? <span className="fwo-sheet-actions"><button className="fwo-sheet-btn" type="button" onClick={downloadCsv}><Download />Download CSV</button><button className="fwo-sheet-btn" type="button" onClick={downloadXlsx}><Download />Download XLSX</button></span> : null}</div>
        </>
      ) : (
        <div className="fwo-sheet-convert"><strong>{rows.length} rows ready</strong><p>{columnCount} columns detected in {fileName}.</p><button className="fwo-sheet-btn primary" type="button" onClick={convert}><Download />{mode === 'csv-to-xlsx' ? 'Download XLSX' : 'Download CSV'}</button></div>
      )}
    </div>
  );
}
