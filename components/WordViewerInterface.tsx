...
export function WordViewerInterface({ toolId }: { toolId: string }) {
  ...
  return <div className="word-viewer-tool">
    ...
    {name || html ? <div className="wvt-top"><div className="wvt-file"><div className="wvt-icon"><FileText/></div><div className="wvt-copy"><strong>{name}</strong><span>{status}</span></div></div><div className="wvt-actions"><button className="wvt-btn" disabled={busy} type="button" onClick={() => inputRef.current?.click()}>{busy ? <RefreshCw size={16}/> : <FolderOpen size={16}/>} {busy ? 'Opening…' : 'Open another'}</button>{name ? <button className="wvt-btn secondary" type="button" onClick={reset}><Trash2 size={16}/>Clear</button> : null}</div></div> : null}
    {warnings.length ? <div className="wvt-warning">{warnings.slice(0,3).join(' · ')}</div> : null}
    {html ? <div className="wvt-frame"><article className="wvt-page" dangerouslySetInnerHTML={{__html:html}} /></div> : <div className="wvt-empty wvt-picker"><button className="wvt-btn" disabled={busy} type="button" onClick={() => inputRef.current?.click()}>{busy ? <RefreshCw size={16}/> : <FolderOpen size={16}/>} {busy ? 'Opening…' : 'Choose Files'}</button></div>}
  </div>;
}
