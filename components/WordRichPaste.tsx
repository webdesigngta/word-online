...
// Deliberately exclude DIV. A4 pages are DIV containers and must never be
// cloned/replaced as though they were text paragraphs during a paste.
...
function blockForNode(editor: HTMLElement, node: Node) {
  const element: HTMLElement | null = node.nodeType === Node.TEXT_NODE
    ? node.parentElement
  ...
  if (block.classList.contains('fwo-page-sheet')) return null;
  return block;
}
...
function insertMultilineAtContainer(editor: HTMLElement, selection: Selection, range: Range, lines: string[]) {
...
function insertPlainText(editor: HTMLElement, text: string) {
  ...
  if (!normalized.includes('\n')) {
    ...
    if (startBlock && startBlock === endBlock) {
      ...
      // This includes legacy/plain text that sits directly in an A4 page.
      // Insert clean paragraph siblings without ever replacing the page wrapper.
      insertMultilineAtContainer(editor, selection, range, lines);
    } else {
      // Cross-block replacement: browser deletes the selected range, but the
      // inserted markup is generated by DOC321 from text/plain only.
 ...
 * - A4 page containers are never treated as formatting blocks
 */
