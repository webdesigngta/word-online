    ...
    try {
      ...
      try {
        for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
          ...
          const name = `${baseName(file.name)}-page-${pageNumber}.${extension}`;
          outputs.push({ name, blob, size: blob.size, pageNumber, type: mime });
          canvas.width = 0;
          canvas.height = 0;
        }
      } finally {
      ...
      return {
        ...
        warnings: pdfResult.warnings.map((item) => item.message),
        errors: [],
      };
    } catch (error) {
